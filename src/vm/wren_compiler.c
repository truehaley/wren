...
static void error(Compiler* compiler, const char* format, ...)
{
  ...
  {
    wrenPrintError(compiler->parser, token->line, "Error at newline", format, args);
  }
  else if (token->type == TOKEN_EOF)
  {
    wrenPrintError(compiler->parser, token->line,
  ...
  else
  {
    ...
    wrenPrintError(compiler->parser, token->line, label, format, args);
  }
  va_end(args);
}
                    ...
                    const char* errorMessage)
{
  ...
  if (compiler->parser->previous.type != expected)
  {
    ...
    if (compiler->parser->current.type == expected) wrenNextToken(compiler->parser);
  }
}
